...
async function updatePage(
  ...
  try {
    response = await notion.pages.update({ page_id: pageID, properties });
  } catch (error) {
    ...
    response = await notion.pages.retrieve({ page_id: pageID });
    // TODO: Need to check if this created a new page. If not, we should throw the error.
  }

  return recreatePageIfDatabaseDiffers(
    notion,
    databaseID,
    properties,
    response,
...
async function recreatePageIfDatabaseDiffers(
  ...
  const currentDatabaseID = response.parent.database_id.replace(/-/g, '');
  if (currentDatabaseID === desiredDatabaseID) {
    return response;
  }
